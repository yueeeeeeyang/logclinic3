...
use crate::{
    ...
    archive::{cleanup_materialized_file, materialize_source_for_paging},
    log_source::LogFileSource,
};

use super::{
    DecodedLogDocument, EncodingChoice, LogContentError, PagedLogDocument, decode_log_bytes,
...
/// - 该阈值只决定“浏览模式”；`log_document::MAX_LOG_FILE_BYTES` 仍作为内存读取硬上限，防止压缩包成员声明大小异常时一次性读爆内存。
