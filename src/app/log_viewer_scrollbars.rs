...
impl MainView {
    ...
    pub(super) fn render_log_vertical_scrollbar_for_tab(
        &self,
        tab: &OpenLogTab,
        context: &mut Context<Self>,
    ) -> gpui::Stateful<gpui::Div> {
    ...
    pub(super) fn render_log_horizontal_scrollbar_for_tab(
        &self,
        tab: &OpenLogTab,
        context: &mut Context<Self>,
    ) -> gpui::Stateful<gpui::Div> {
    ...
    pub(super) fn paged_log_estimated_content_width(
    ...
    /// 返回分页日志横向最大逻辑滚动距离。
    pub(super) fn paged_log_horizontal_max_scroll_px(
        document: &log_document::PagedLogDocument,
    ...
    /// 计算分页日志纵向滚动条滑块。
    pub(super) fn paged_log_vertical_scrollbar_metrics(
        tab: &OpenLogTab,
        document: &log_document::PagedLogDocument,
    ) -> Option<LogScrollbarMetrics> {
    ...
    /// 计算分页日志横向滚动条滑块。
    pub(super) fn paged_log_horizontal_scrollbar_metrics(
        tab: &OpenLogTab,
        document: &log_document::PagedLogDocument,
        font_size: f32,
    ) -> Option<LogScrollbarMetrics> {
