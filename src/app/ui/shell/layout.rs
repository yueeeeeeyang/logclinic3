...
/// 单行输入框自动水平滚动时保留在光标两侧的可见边距。
///
/// 业务意图：
/// - 搜索关键字、当前目录、快搜关键字和模型配置字段都使用自绘单行输入；当文本超过可视宽度时，
///   光标不能贴着输入框边缘跳动，保留少量边距更接近系统输入框的横向滚动体验。
/// - 边距只影响文本绘制偏移，不改变输入框外观尺寸、字号或布局样式。
pub(in crate::app) const SINGLE_LINE_INPUT_SCROLL_MARGIN: f32 = 8.0;

/// 单行输入框自绘光标宽度。
///
/// 业务意图：
/// - 搜索关键字、当前目录、快搜关键字和模型配置字段共用自绘文本输入；光标宽度必须集中维护，
///   否则滚动计算和绘制矩形容易出现不一致，导致光标在输入框最右侧被裁剪。
/// - 该常量只描述已有绘制宽度，不改变输入框字号、边框或整体布局。
pub(in crate::app) const SINGLE_LINE_INPUT_CARET_WIDTH: f32 = 1.5;
...
///
/// 业务意图：
/// - 搜索栏属于目录树内部导航工具，需要紧贴标题和文件列表之间，帮助用户在大目录中快速收敛到目标文件。
/// - 高度控制在一行工具条内，避免挤占右侧日志正文和左侧文件树的可视空间。
///
/// 边界条件：
/// - 搜索栏不参与虚拟列表行高计算；文件行仍由 `LOG_TREE_ROW_HEIGHT` 独立控制。
pub(in crate::app) const LOG_TREE_SEARCH_BAR_HEIGHT: f32 = 36.0;

/// 左侧目录树搜索输入框高度。
///
/// 业务意图：
/// - 输入框需要容纳 12px 树字体、IME 下划线和自绘光标，同时保持和目录树整体密度一致。
/// - 使用固定高度可以避免 macOS 和 Windows 字体度量差异导致搜索栏上下跳动。
pub(in crate::app) const LOG_TREE_SEARCH_INPUT_HEIGHT: f32 = 24.0;

/// 左侧目录树搜索栏主控件之间的水平间距。
///
/// 业务意图：
/// - 输入关键字后右侧会出现命中计数和上下导航按钮，间距需要比普通工具栏更紧凑，避免窄侧栏里输入框被过度挤压。
pub(in crate::app) const LOG_TREE_SEARCH_BAR_ITEM_GAP: f32 = 3.0;

/// 左侧目录树搜索命中导航组内的水平间距。
///
/// 业务意图：
/// - 计数、上一个、下一个属于同一组控件，使用更小间距让它们在视觉上聚合为搜索导航，而不是分散的独立按钮。
pub(in crate::app) const LOG_TREE_SEARCH_NAV_GROUP_GAP: f32 = 2.0;

/// 左侧目录树搜索命中计数的最小宽度。
///
/// 业务意图：
/// - 计数不再占用固定大宽度，按文本自然宽度展示；保留最小宽度只为避免 0/0、1/9 等短文本贴近按钮。
pub(in crate::app) const LOG_TREE_SEARCH_MATCH_COUNT_MIN_WIDTH: f32 = 28.0;

/// 左侧目录树搜索栏图标按钮边长。
///
/// 业务意图：
/// - 清空、上一个和下一个命中都是高频小操作，固定方形命中区能保证窄侧栏下仍容易点击。
/// - 搜索栏空间有限，按钮尺寸略小于输入框高度，使右侧导航组更紧凑，同时仍保留足够鼠标命中区。
pub(in crate::app) const LOG_TREE_SEARCH_BUTTON_SIZE: f32 = 20.0;
