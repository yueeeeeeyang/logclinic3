...
// - 当前通过独立 `impl MainView` 保持行为不变，同时把目录树 GPUI 视图从应用根文件中分离出来。
//
// 边界条件：
...
impl MainView {
    ...
    pub(in crate::app) fn render_log_tree_header(&self) -> impl IntoElement {
        let palette = self.palette();
    ...
    pub(in crate::app) fn render_log_tree_body(
        &self,
        context: &mut Context<Self>,
    ) -> impl IntoElement {
    ...
    pub(in crate::app) fn log_tree_scrollbar_metrics(
    ...
    pub(in crate::app) fn render_loaded_log_tree_row(
        &self,
        visible_index: usize,
    ...
    pub(in crate::app) fn render_log_tree_row(
            ...
            .on_mouse_down(
                ...
                context.listener(move |view, event: &MouseDownEvent, _window, context| {
                    view.open_log_tree_context_menu(
                        LogTreeContextMenuRequest {
                            node_id,
                            visible_index,
                            source: source_for_right_click.clone(),
    ...
    pub(in crate::app) fn log_tree_row_hover_background(
        selected: bool,
        theme: EffectiveTheme,
    ) -> u32 {
    ...
    pub(in crate::app) fn handle_log_tree_left_mouse_down(
        &mut self,
        node_id: usize,
        visible_index: usize,
    ...
    pub(in crate::app) fn log_tree_primary_action_for_click(
        has_source: bool,
        can_toggle: bool,
        shift: bool,
    ...
    pub(in crate::app) fn update_log_tree_selection_for_click(
        &mut self,
        node_id: usize,
        visible_index: usize,
        shift: bool,
    ...
    pub(in crate::app) fn apply_log_tree_selection_click(
    ...
    pub(in crate::app) fn visible_log_tree_node_ids(&self) -> Vec<usize> {
    ...
    /// - 参数对象直接来自鼠标事件和树节点快照，可以避免菜单打开时再次按 ID 查询易变行。
    pub(in crate::app) fn open_log_tree_context_menu(
        &mut self,
        request: LogTreeContextMenuRequest,
        event: &MouseDownEvent,
        context: &mut Context<Self>,
    ) {
        let LogTreeContextMenuRequest {
            node_id,
            visible_index,
            source,
            window_x,
    ...
    pub(in crate::app) fn render_log_tree_context_menu(
        &self,
        context: &mut Context<Self>,
    ) -> gpui::Stateful<gpui::Div> {
    ...
    pub(in crate::app) fn render_log_tree_context_menu_dismiss_overlay(
        &self,
        context: &mut Context<Self>,
    ) -> gpui::Stateful<gpui::Div> {
    ...
    /// - 渲染 helper 需要同时拿到节点、动作、图标和主题，参数对象让调用处保持命名字段，避免位置参数误传。
    pub(in crate::app) fn render_log_tree_context_menu_item(
        &self,
        request: LogTreeContextMenuItemRequest,
        context: &mut Context<Self>,
    ) -> gpui::Stateful<gpui::Div> {
        let LogTreeContextMenuItemRequest {
            node_id,
            fallback_source,
            action,
            label,
    ...
    pub(in crate::app) fn handle_log_tree_context_menu_action(
        &mut self,
    ...
    pub(in crate::app) fn selected_log_tree_file_sources(&self) -> Vec<LogFileSource> {
    ...
    pub(in crate::app) fn save_selected_log_tree_sources_as(
        &mut self,
    ...
    pub(in crate::app) fn save_log_sources_as(
        &mut self,
        sources: Vec<LogFileSource>,
        context: &mut Context<Self>,
    ) {
    ...
    pub(in crate::app) fn spawn_save_log_sources_to_directory(
        &mut self,
        sources: Vec<LogFileSource>,
    ...
    pub(in crate::app) fn save_log_sources_to_directory(
    ...
    pub(in crate::app) fn save_target_conflicts(
        sources: &[LogFileSource],
        target_directory: &Path,
    ) -> Vec<PathBuf> {
        sources
    ...
    pub(in crate::app) fn handle_save_overwrite_choice(
        &mut self,
        conflict_policy: SaveConflictPolicy,
        context: &mut Context<Self>,
    ) {
    ...
    pub(in crate::app) fn save_relative_path_for_source(source: &LogFileSource) -> PathBuf {
        pub(in crate::app) fn file_name_from_member_path(member_path: &str) -> PathBuf {
            member_path
                .split('/')
    ...
    pub(in crate::app) fn should_cleanup_saved_materialized_source(
