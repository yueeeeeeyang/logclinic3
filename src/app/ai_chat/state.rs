...
// - 方法仍实现到 MainView 上，保持和其它 app 功能域一致的调用方式，不引入新的公开状态对象。
...
impl MainView {
    ...
    pub(in crate::app) fn ai_chat_can_send(&self) -> bool {
        self.ai_chat.streaming_task.is_none()
            ...
            && !self.ai_chat.input_text.trim().is_empty()
    }
    ...
    pub(in crate::app) fn create_ai_chat_conversation(&mut self, context: &mut Context<Self>) {
        ...
        match insert_ai_chat_conversation(&path, &conversation) {
            Ok(()) => {
                self.ai_chat.conversations.insert(0, conversation.clone());
                self.ai_chat.active_conversation_id = Some(conversation.id);
                self.ai_chat.messages.clear();
                self.ai_chat.input_text.clear();
                self.ai_chat.input_selection_range = 0..0;
            ...
            Err(error) => self.ai_chat.database_error = Some(error),
        }
        context.notify();
    }
    ...
    pub(in crate::app) fn select_ai_chat_conversation(
        ...
        match load_ai_chat_messages(&path, conversation_id) {
            Ok(messages) => {
                ...
                self.ai_chat.active_conversation_id = Some(conversation_id.to_string());
                self.ai_chat.messages = messages;
                self.ai_chat.model_menu_open = false;
                self.ai_chat.database_error = None;
                self.reset_ai_chat_message_list_state();
            }
        ...
        match delete_ai_chat_conversation(&path, &active_id) {
            Ok(()) => {
                ...
                if self.ai_chat.conversations.is_empty() {
                    ...
                    match insert_ai_chat_conversation(&path, &conversation) {
                        Ok(()) => {
                            self.ai_chat.active_conversation_id = Some(conversation.id.clone());
                            self.ai_chat.conversations.push(conversation);
                            self.ai_chat.messages.clear();
                            self.ai_chat.database_error = None;
                            self.reset_ai_chat_conversation_list_state();
                            self.reset_ai_chat_message_list_state();
                        ...
                        Err(error) => {
                            self.ai_chat.active_conversation_id = None;
                            self.ai_chat.messages.clear();
                            self.ai_chat.database_error = Some(error);
                            self.reset_ai_chat_conversation_list_state();
                            self.reset_ai_chat_message_list_state();
                        }
                    }
                } else {
                    let next_id = self.ai_chat.conversations[0].id.clone();
                    self.ai_chat.active_conversation_id = Some(next_id.clone());
                    self.ai_chat.messages =
    ...
    pub(in crate::app) fn select_ai_chat_model_profile(
        ...
        {
            self.ai_chat.database_error = Some("选择的模型配置不存在".to_string());
            context.notify();
            return;
        }
    ...
    pub(in crate::app) fn sort_ai_chat_conversations(&mut self) {
        self.ai_chat.conversations.sort_by(|left, right| {
            right
                .updated_at_ms
    ...
    pub(in crate::app) fn reset_ai_chat_conversation_list_state(&mut self) {
        self.ai_chat
            ...
            .reset(self.ai_chat.conversations.len());
        if self
            .ai_chat
            .scrollbar_drag
    ...
    pub(in crate::app) fn ai_chat_list_state_for_area(&self, area: AiChatScrollArea) -> ListState {
        match area {
            AiChatScrollArea::Conversations => self.ai_chat.conversation_list_state.clone(),
            AiChatScrollArea::Messages => self.ai_chat.message_list_state.clone(),
        }
    }
    ...
    pub(in crate::app) fn start_ai_chat_scrollbar_drag(
        ...
        self.ai_chat.scrollbar_drag = Some(AiChatScrollbarDrag {
            area,
    ...
    pub(in crate::app) fn update_ai_chat_scrollbar_drag(
        ...
        let Some(drag) = self.ai_chat.scrollbar_drag else {
            return;
        };
        if !event.dragging() {
    ...
    pub(in crate::app) fn stop_ai_chat_scrollbar_drag(&mut self, context: &mut Context<Self>) {
        let Some(drag) = self.ai_chat.scrollbar_drag.take() else {
            return;
        };
    ...
    pub(in crate::app) fn start_ai_chat_input_resize(&mut self, event: &MouseDownEvent) {
        self.ai_chat.input_resize_drag = Some(AiChatInputResizeDrag {
    ...
    pub(in crate::app) fn update_ai_chat_input_resize_drag(
        ...
        let Some(drag) = self.ai_chat.input_resize_drag else {
            return;
        };
        if !event.dragging() {
    ...
    pub(in crate::app) fn stop_ai_chat_input_resize_drag(&mut self, context: &mut Context<Self>) {
        if self.ai_chat.input_resize_drag.take().is_some() {
            context.notify();
        }
    }
    ...
    pub(in crate::app) fn ai_chat_input_visual_line_count(&self) -> usize {
        Self::thread_analysis_filter_line_ranges(&self.ai_chat.input_text)
            .len()
            .max(1)
    }
    ...
    pub(in crate::app) fn ai_chat_input_index_for_point(&self, position: Point<Pixels>) -> usize {
        if self.ai_chat.input_text.is_empty() {
            return 0;
        }
        ...
        if let Some(bounds) = &self.ai_chat.input_last_bounds
            && position.y < bounds.top()
        {
            return 0;
        }
        self.ai_chat.input_text.len()
    }
        ...
        match event.click_count {
            0 | 1 => {
                if event.modifiers.shift {
                    self.ai_chat.input_selection_range.end = index;
                    self.ai_chat.input_selection_range = Self::clamp_search_text_range(
                        &self.ai_chat.input_text,
                        self.ai_chat.input_selection_range.clone(),
                    );
                } else {
                    self.ai_chat.input_selection_range = index..index;
                }
                self.ai_chat.input_selection_drag = Some(self.ai_chat.input_selection_range.start);
            }
            2 => {
                self.ai_chat.input_selection_range =
                    Self::search_text_word_range_for_index(&self.ai_chat.input_text, index);
                self.ai_chat.input_selection_drag = None;
            }
            _ => {
                self.ai_chat.input_selection_range = 0..self.ai_chat.input_text.len();
                self.ai_chat.input_selection_drag = None;
            }
        }
    ...
    pub(in crate::app) fn update_ai_chat_input_mouse_selection(
        ...
        let Some(anchor) = self.ai_chat.input_selection_drag else {
            return;
        };
        let index = self.ai_chat_input_index_for_point(position);
        self.ai_chat.input_marked_range = None;
        self.ai_chat.input_selection_range =
            Self::clamp_search_text_range(&self.ai_chat.input_text, anchor..index);
        self.touch_search_text_cursor_activity();
        context.notify();
    }
    ...
    pub(in crate::app) fn finish_ai_chat_input_mouse_selection(
        ...
        if self.ai_chat.input_selection_drag.take().is_some() {
            context.notify();
        }
    }
    ...
    pub(in crate::app) fn selected_ai_chat_input_text(&self) -> Option<String> {
        ...
        (range.start < range.end).then(|| self.ai_chat.input_text[range].to_string())
    }

    /// 用给定文本替换 AI 对话输入区当前选区。
    pub(in crate::app) fn replace_ai_chat_input_selection(&mut self, replacement: &str) {
        let replacement = replacement.replace("\r\n", "\n").replace('\r', "\n");
        let range = self.ai_chat.input_marked_range.take().unwrap_or_else(|| {
            Self::clamp_search_text_range(
                &self.ai_chat.input_text,
                self.ai_chat.input_selection_range.clone(),
            )
    ...
    pub(in crate::app) fn handle_ai_chat_input_key_down(
        ...
        match event.keystroke.key.as_str() {
            ...
            "left" => {
                self.ai_chat.input_marked_range = None;
                if event.keystroke.modifiers.shift {
                    self.ai_chat.input_selection_range.end = Self::previous_search_text_boundary(
                        &self.ai_chat.input_text,
                        self.ai_chat.input_selection_range.end,
                    );
                    self.ai_chat.input_selection_range = Self::clamp_search_text_range(
                        &self.ai_chat.input_text,
                        self.ai_chat.input_selection_range.clone(),
                    );
                } else if self.ai_chat.input_selection_range.start
                    != self.ai_chat.input_selection_range.end
                {
                    self.ai_chat.input_selection_range = self.ai_chat.input_selection_range.start
                        ..self.ai_chat.input_selection_range.start;
                } else {
            ...
            "right" => {
                self.ai_chat.input_marked_range = None;
                if event.keystroke.modifiers.shift {
                    self.ai_chat.input_selection_range.end = Self::next_search_text_boundary(
                        &self.ai_chat.input_text,
                        self.ai_chat.input_selection_range.end,
                    );
                    self.ai_chat.input_selection_range = Self::clamp_search_text_range(
                    ...
                    != self.ai_chat.input_selection_range.end
                {
                    self.ai_chat.input_selection_range = self.ai_chat.input_selection_range.end
                        ..self.ai_chat.input_selection_range.end;
                } else {
            ...
            "down" => {
                self.ai_chat.input_marked_range = None;
                let cursor = self.ai_chat.input_text.len();
                self.ai_chat.input_selection_range = cursor..cursor;
                self.touch_search_text_cursor_activity();
                context.stop_propagation();
                context.notify();
            ...
            "backspace" => {
                if self.ai_chat.input_selection_range.start
                    != self.ai_chat.input_selection_range.end
                    || self.ai_chat.input_marked_range.is_some()
                {
                    self.replace_ai_chat_input_selection("");
                } else if let Some((previous_index, _)) = self.ai_chat.input_text
                    [..self.ai_chat.input_selection_range.end]
                    .char_indices()
                    .next_back()
                {
                    let cursor = self.ai_chat.input_selection_range.end;
                    self.ai_chat
                        .input_text
                        .replace_range(previous_index..cursor, "");
                    self.ai_chat.input_selection_range = previous_index..previous_index;
                    self.ai_chat.input_marked_range = None;
                }
                self.touch_search_text_cursor_activity();
                context.stop_propagation();
            ...
            "delete" => {
                ...
                {
                    self.replace_ai_chat_input_selection("");
                } else if let Some((next_index, next_character)) = self.ai_chat.input_text
                    [self.ai_chat.input_selection_range.end..]
                    .char_indices()
                    .next()
                {
                    let start = self.ai_chat.input_selection_range.end + next_index;
                    let end = start + next_character.len_utf8();
                    self.ai_chat.input_text.replace_range(start..end, "");
                    self.ai_chat.input_selection_range = start..start;
                    self.ai_chat.input_marked_range = None;
                }
                self.touch_search_text_cursor_activity();
                context.stop_propagation();
    ...
    pub(in crate::app) fn start_ai_chat_send(&mut self, context: &mut Context<Self>) {
        ...
        let Some(profile) = self.active_ai_chat_model_profile() else {
            self.ai_chat.database_error = Some("需要选择一个可用模型配置".to_string());
            context.notify();
            return;
        };

        let content = self.ai_chat.input_text.trim().to_string();
        ...
        let old_message_count = self.ai_chat.messages.len();
        self.ai_chat.messages.push(user_message.clone());
        self.ai_chat.messages.push(assistant_message.clone());
        self.ai_chat
            .message_list_state
            .splice(old_message_count..old_message_count, 2);
        self.ai_chat
            .message_list_state
            .scroll_to_reveal_item(self.ai_chat.messages.len().saturating_sub(1));
        self.ai_chat.input_text.clear();
        self.ai_chat.input_selection_range = 0..0;
        self.ai_chat.input_marked_range = None;
        self.ai_chat.database_error = None;
        ...
        let request_messages = self.ai_chat.messages.clone();
        let (sender, receiver) = mpsc::channel();
        let cancel = Arc::new(AtomicBool::new(false));
        let cancel_for_task = cancel.clone();
        let job_id = self.ai_chat.next_job_id;
        self.ai_chat.next_job_id = self.ai_chat.next_job_id.saturating_add(1);
        self.ai_chat.streaming_task = Some(AiChatStreamingTask {
            job_id,
            conversation_id,
    ...
    pub(in crate::app) fn finish_ai_chat_streaming_task_immediately(
        ...
        let Some(task) = self.ai_chat.streaming_task.take() else {
            return;
        };
                ...
                loop {
                    ...
                    let keep_polling = view
                        .update(app, |view, context| {
                            ...
                            view.ai_chat.streaming_task.is_some()
                        })
                        .unwrap_or(false);
                    if !keep_polling {
                        break;
    ...
    pub(in crate::app) fn apply_ai_chat_stream_event(&mut self, event: AiChatStreamEvent) {
        let Some(task_snapshot) = self.ai_chat.streaming_task.as_ref().map(|task| {
            (
        ...
        if self.ai_chat.active_conversation_id.as_deref() != Some(conversation_id.as_str()) {
            return;
        }
        ...
        match event {
            ...
            AiChatStreamEvent::Done => {
                ...
                self.ai_chat.streaming_task = None;
            }
