...
pub(super) struct NavigationState {
    /// 当前主窗口选中的大功能页。
    pub(super) active_main_feature: MainFeature,
    /// 当前鼠标悬浮的左侧导航入口。
    pub(super) hovered_navigation_item: Option<MainNavigationItem>,
    /// 最近一次鼠标所在或点击的键盘滚动区域。
    pub(super) keyboard_scroll_region: Option<KeyboardScrollRegion>,
}
...
pub(super) struct LogWorkspaceState {
    /// 左侧日志目录树当前的数据状态。
    pub(super) load_state: LogTreeLoadState,
    /// 左侧目录树虚拟列表的滚动句柄。
    pub(super) log_tree_scroll_handle: UniformListScrollHandle,
    /// 左侧目录树当前选中的节点 ID 集合。
    pub(super) log_tree_selected_node_ids: HashSet<usize>,
    /// Shift 多选的锚点节点 ID。
    pub(super) log_tree_selection_anchor: Option<usize>,
    /// 当前打开的左侧目录树右键菜单。
    pub(super) log_tree_context_menu: Option<LogTreeContextMenu>,
    /// 右侧 tab 栏横向滚动句柄。
    pub(super) tab_bar_scroll_handle: ScrollHandle,
    /// 右侧已经打开的日志 tab 列表。
    pub(super) open_tabs: Vec<OpenLogTab>,
    /// 当前激活的日志 tab ID。
    ...
    /// 当前打开的加载日志来源菜单。
    pub(super) load_source_menu: Option<LoadSourceMenu>,
    /// 当前打开的编码下拉框。
    pub(super) encoding_dropdown_menu: Option<EncodingDropdownMenu>,
    /// 当前打开的日志正文右键菜单。
    pub(super) log_viewer_context_menu: Option<LogViewerContextMenu>,
    /// 另存为同名文件覆盖确认弹窗。
    pub(super) save_overwrite_confirm_dialog: Option<SaveOverwriteConfirmDialog>,
    /// 日志正文自绘滚动条的拖动状态。
    pub(super) log_scrollbar_drag: Option<LogScrollbarDrag>,
    /// 左侧目录树自绘滚动条的拖动状态。
    pub(super) log_tree_scrollbar_drag: Option<LogTreeScrollbarDrag>,
}
...
pub(super) struct SearchWorkspaceState {
    /// 当前搜索对话框状态。
    pub(super) search_dialog: Option<SearchDialogState>,
    /// 当前会话内的搜索关键字历史。
    pub(super) search_query_history: Vec<SearchQueryHistoryItem>,
    /// 搜索对话框独立窗口句柄。
    pub(super) search_dialog_window: Option<WindowHandle<SearchDialogWindowView>>,
    /// 搜索对话框打开请求是否已经排队到下一帧。
    pub(super) search_dialog_open_pending: bool,
    /// 搜索结果底部面板状态。
    pub(super) search_results_panel: Option<SearchResultsPanelState>,
    /// 搜索结果面板高度拖动状态。
    pub(super) search_results_resize_drag: Option<SearchResultsResizeDrag>,
    /// 搜索结果面板滚动条拖动状态。
    pub(super) search_results_scrollbar_drag: Option<SearchResultsScrollbarDrag>,
    /// 当前打开的搜索结果右键菜单。
    pub(super) search_results_context_menu: Option<SearchResultsContextMenu>,
    /// 下一个搜索任务 ID。
    pub(super) next_search_job_id: usize,
    /// 搜索输入框焦点句柄。
    pub(super) search_input_focus: gpui::FocusHandle,
    /// 当前目录搜索目标输入框焦点句柄。
    pub(super) search_directory_focus: gpui::FocusHandle,
    /// 搜索关键字输入框最近一次排版结果。
    pub(super) search_query_last_layout: Option<ShapedLine>,
    /// 搜索关键字输入框最近一次绘制边界。
    pub(super) search_query_last_bounds: Option<Bounds<Pixels>>,
    /// 目录目标输入框最近一次排版结果。
    pub(super) search_directory_last_layout: Option<ShapedLine>,
    /// 目录目标输入框最近一次绘制边界。
    pub(super) search_directory_last_bounds: Option<Bounds<Pixels>>,
    /// 当前正在拖拽选择的搜索文本输入槽位。
    pub(super) search_text_selection_drag: Option<(SearchTextInputKind, usize)>,
    /// 搜索输入框最近一次光标活动时间。
    pub(super) search_text_cursor_last_activity: Instant,
}
...
impl SearchWorkspaceState {
    ...
    pub(super) fn new(context: &mut Context<MainView>) -> Self {
        Self {
            search_dialog: None,
...
pub(super) struct SettingsState {
    /// 设置窗口独立窗口句柄。
    pub(super) settings_window: Option<WindowHandle<SettingsWindowView>>,
    /// 设置窗口打开请求是否已经排队到下一帧。
    pub(super) settings_window_open_pending: bool,
    /// 设置窗口当前激活页签。
    pub(super) settings_active_tab: SettingsTab,
    /// 当前主题偏好。
    pub(super) theme_preference: ThemePreference,
    /// 日志正文显示字号。
    pub(super) log_viewer_font_size: f32,
    /// 线程日志分析过滤配置原文。
    pub(super) thread_analysis_filter_text: String,
    /// 线程日志分析过滤输入区是否处于编辑状态。
    pub(super) thread_analysis_filter_is_editing: bool,
    /// 进入编辑前的线程日志分析过滤配置快照。
    pub(super) thread_analysis_filter_saved_text_before_edit: Option<String>,
    /// 快搜关键字单行输入框状态。
    pub(super) quick_search_keywords_input: SingleLineTextInputState,
    /// 快搜关键字输入区是否处于编辑状态。
    pub(super) quick_search_keywords_is_editing: bool,
    /// 进入编辑前的快搜关键字配置快照。
    pub(super) quick_search_keywords_saved_text_before_edit: Option<String>,
    /// 快搜关键字输入区焦点句柄。
    pub(super) quick_search_keywords_focus: gpui::FocusHandle,
    /// 快搜关键字输入区最近一次单行排版结果。
    pub(super) quick_search_keywords_last_layout: Option<ShapedLine>,
    /// 快搜关键字输入区最近一次绘制边界。
    pub(super) quick_search_keywords_last_bounds: Option<Bounds<Pixels>>,
    /// 快搜关键字输入区拖拽选择锚点。
    pub(super) quick_search_keywords_selection_drag: Option<usize>,
    /// 线程日志分析过滤输入区的选择范围。
    pub(super) thread_analysis_filter_selection_range: Range<usize>,
    /// 线程日志分析过滤输入区的输入法组合文本范围。
    pub(super) thread_analysis_filter_marked_range: Option<Range<usize>>,
    /// 线程日志分析过滤输入区焦点句柄。
    pub(super) thread_analysis_filter_focus: gpui::FocusHandle,
    /// 线程日志分析过滤输入区最近一次绘制的逐行布局。
    pub(super) thread_analysis_filter_last_layouts: Vec<ThreadAnalysisFilterLineLayout>,
    /// 线程日志分析过滤输入区最近一次整体绘制边界。
    pub(super) thread_analysis_filter_last_bounds: Option<Bounds<Pixels>>,
    /// 线程日志分析过滤输入区拖拽选择锚点。
    pub(super) thread_analysis_filter_selection_drag: Option<usize>,
}
...
pub(super) struct ModelConfigState {
    /// 已保存的模型配置列表。
    pub(super) model_config_profiles: Vec<ModelProfile>,
    /// 当前默认模型配置 ID。
    pub(super) model_config_default_profile_id: Option<String>,
    /// 当前在模型配置列表中选中的已保存配置 ID。
    pub(super) model_config_selected_profile_id: Option<String>,
    /// 当前表单对应的已保存配置 ID。
    pub(super) model_config_form_profile_id: Option<String>,
    /// 模型配置名称输入框状态。
    pub(super) model_config_name_input: ModelConfigTextFieldState,
    /// 模型 Base URL 输入框状态。
    pub(super) model_config_base_url_input: ModelConfigTextFieldState,
    /// 模型 API Key 输入框状态。
    pub(super) model_config_api_key_input: ModelConfigTextFieldState,
    /// 模型 ID 输入框状态。
    pub(super) model_config_model_input: ModelConfigTextFieldState,
    /// API Key 是否在 UI 中明文显示。
    pub(super) model_config_api_key_visible: bool,
    /// 模型测试请求状态。
    pub(super) model_test_status: ModelTestStatus,
    /// 下一个模型测试任务 ID。
    pub(super) next_model_test_job_id: usize,
}
...
impl ModelConfigState {
    ...
    pub(super) fn new(context: &mut Context<MainView>, model_configs: ModelConfigs) -> Self {
