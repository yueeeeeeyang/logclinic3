...
pub(crate) mod shader_resources {
    ...
    impl<'t> RawShaderBytes<'t> {
        pub(crate) fn new(module: ShaderModule, target: ShaderTarget) -> Result<Self> {
            // 本地补丁：上游 release 构建依赖 `fxc.exe` 在 build.rs 中预编译 HLSL，
            // 但 macOS/Linux 通过 cargo-xwin 交叉编译时 build.rs 是宿主平台，无法进入 Windows 专用模块。
            // 这里统一在运行时用 Windows 自带的 d3dcompiler_47 编译内嵌 HLSL 源码，保证交叉编译产物不依赖宿主机 fxc。
    ...
    pub(super) fn build_shader_blob(module: ShaderModule, target: ShaderTarget) -> Result<ID3DBlob> {
        unsafe {
            let shader_name = if matches!(module, ShaderModule::EmojiRasterization) {
            ...
            // 本地补丁：把唯一的 HLSL include 在编译前内联，避免运行时依赖上游源码目录路径。
            // Windows 运行环境只需要系统 `d3dcompiler_47.dll`，不需要携带 `.hlsl` 文件。
            let shader_source = shader_source_for_module(module);
            let shader_source_name = format!("{shader_name}\0");
            let mut compile_blob = None;
            let mut error_blob = None;

            let entry_point = PCSTR::from_raw(entry.as_ptr());
            let target_cstr = PCSTR::from_raw(target.as_ptr());
            let source_name = PCSTR::from_raw(shader_source_name.as_ptr());
    ...
    fn shader_source_for_module(module: ShaderModule) -> Vec<u8> {
        const INCLUDE_DIRECTIVE: &str = "#include \"alpha_correction.hlsl\"";
        let alpha_correction = include_str!("alpha_correction.hlsl");
        let source = if matches!(module, ShaderModule::EmojiRasterization) {
            include_str!("color_text_raster.hlsl")
        } else {
            include_str!("shaders.hlsl")
        };
        let inlined_source = source.replace(INCLUDE_DIRECTIVE, alpha_correction);
        inlined_source.into_bytes()
    }
